...
build_imx_uboot()
{
	...
	cp ${UBOOT_OUT}/arch/arm/dts/maaxboard.dtb  ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
	...
	if [ `echo $2 | rev | cut -d '-' -f1 | rev` != "dual" ]; then
		...
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8M flash_ddr4_val_dual_bootloader || exit 1
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8M PRINT_FIT_HAB_OFFSET=0x0 print_fit_hab_ddr4 || exit 1
	fi
	if [ `echo $2 | rev | cut -d '-' -f1 | rev` != "dual" ]; then
